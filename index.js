...
(function () {
	...
	var PhyTouch = function (option) {
		...
		this.followers = followersArr.map(function (follower) {
			return {
				element:
					typeof follower.element === "string" ? document.querySelector(follower.element) : follower.element,
					...
					if (dt < 300) {
						...
						if (destination < this.min) {
							if (destination < this.min - this.maxRegion) {
								tRatio = reverseEase(
									(current - this.min + this.springMaxRegion) / (current - destination)
								);
								...
								destination =
									this.min - (this.springMaxRegion * (this.min - destination)) / this.maxRegion;
							}
						} else if (destination > this.max) {
							if (destination > this.max + this.maxRegion) {
								tRatio = reverseEase(
									(this.max + this.springMaxRegion - current) / (destination - current)
								);
								...
								destination =
									this.max + (this.springMaxRegion * (destination - this.max)) / this.maxRegion;
							}
						}
